...
void snrSEN55(uint8_t *b,struct Sen55_Values *S)
{
for(uint8_t i=0;i<20;++i)
  {
    if(b[i]!=0) S->snr[i] = b[i];
    else i=20;
  }
}

// start I2C wire, read Sensor data
boolean InitSEN55(struct Sen55_Values *S)
{
MyWire.end();
MyWire.begin();
  ...
  if(ReadSEN55(SEN55_SENSORREAD) )
    {
      ...
      //DEBUGF(" * Sensor Read, Status 0x%x\n",S->status)
      return(true);
    }
  else return(false);
  }
